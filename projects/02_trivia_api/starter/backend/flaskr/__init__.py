...
def create_app(test_config=None):
  ...
  def create_quiz():
      ...
      try:
          if cat['id'] == 0:
              questions = Question.query.filter(Question.id.notin_(previous_questions)).limit(1).all()
          else:
              questions = Question.query.filter(Question.category == cat['id'], Question.id.notin_(previous_questions)).limit(1).all()
          if len(questions) > 0:
