...
def venues():
 ...
 vstates = Venue.query.with_entities(Venue.state).group_by(Venue.state).all()
 resultList = []
 for st in vstates:
     fset={}
     venstate=[]
     ...
     for ven in venstate:
         venue={}
         show = Show.query.filter(Show.venue_id==ven.id, Show.show_time>datetime.today()).count()
         venue["id"]=ven.id
         venue["name"]=ven.name
...
def show_venue(venue_id):
  ...
  pshowcnt = Show.query.filter(Show.venue_id==venue_id, Show.show_time<datetime.today()).count()
  upshowcnt = Show.query.filter(Show.venue_id==venue_id, Show.show_time>datetime.today()).count()
  pshowinfo = Show.query.with_entities(Show.show_time,Show.artist_id).filter(Show.venue_id==venue_id,Show.show_time<datetime.today())
  upshowinfo = Show.query.with_entities(Show.show_time,Show.artist_id).filter(Show.venue_id==venue_id,Show.show_time>datetime.today())
...
def create_venue_submission():
    ...
    venue = Venue(name=vname,city=vcity,state=vstate,phone=vphone,address=vaddress, genres=vgenres,facebook_link=vfblink)
    try:
...
def delete_venue(venue_id):
  ...
  try:
      ...
      qry.delete()
      db.session.commit()
      flash('Venue with venue ID ' + venue_id + ' was successfully deleted!')
  except Exception as e:
      flash('Error Occured: Venue with venue ID ' + venue_id + ' could not be deleted.')
  return render_template('pages/home.html')
...
def search_artists():
  ...
  sch_term = request.form.get('search_term','')
  if sch_term:
      #test = Venue.query.filter(func.lower(Venue.name) ==func.lower(sch_term))
      artists = Artist.query.filter(Artist.name.ilike('%'+sch_term+'%'))
...
def show_artist(artist_id):
  ...
  pshowcnt = Show.query.filter(Show.artist_id==artist_id, Show.show_time<datetime.today()).count()
  upshowcnt = Show.query.filter(Show.artist_id==artist_id, Show.show_time>datetime.today()).count()
  pshowinfo = Show.query.with_entities(Show.show_time,Show.venue_id).filter(Show.artist_id==artist_id,Show.show_time<datetime.today())
  upshowinfo = Show.query.with_entities(Show.show_time,Show.venue_id).filter(Show.artist_id==artist_id,Show.show_time>datetime.today())
...
def create_artist_submission():
    ...
    try:
        artist = Artist(name=aname,city=acity,state=astate, phone=aphone, genres=agenres, facebook_link=afblink)
